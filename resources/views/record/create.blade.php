<x-home-master>
    @section('content')
        <div class="card mt-5">
            <h5 class="card-header">Dodaj inspekcijsku kontrolu</h5>
            <div class="card-body">
                <form method="POST" action="{{route('store.record')}}">
                    @csrf
                    <div class="mb-3">
                        <input type="datetime-local" id="date" class="form-control"
                               name="date" required
                               autocomplete="date" autofocus/>
                        <label class="form-label mt-2" for="name">Datum kontrole</label>
                    </div>
                    <div class="mb-3">
